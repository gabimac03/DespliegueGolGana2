...
const FormularioReserva = () => {
  ...
  useEffect(() => {
    const obtenerCancha = async () => {
      try {
        const res = await axios.get(`http://34.8.16.245/api/canchas/${id}`);
        setCancha(res.data);
      } catch {
    ...
    const obtenerReservas = async () => {
      ...
      try {
        const res = await axios.get(`http://34.8.16.245/api/reservas/canchas/${id}/semana?inicio=${lunes}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const lunes = dayjs().startOf('week').add(1, 'day').format('YYYY-MM-DD');
      const res = await axios.get(`/api/reservas/canchas/${id}/semana?inicio=${lunes}`);
